...
<head>
    ...
    <title>Surat Permohonan Cuti - {{ $leave->employee->user->name ?? 'Draft' }}</title>
    @php
...
<body>
    @php
        ...
        $variableValues = [
            ...
            'cuti_selama_berapa_hari' => $leave->total_days ?? '',
            'terhitung_sejak_tanggal' => $leave->start_date ? $leave->start_date->translatedFormat('d F Y') : '',
            'selesai_cuti' => $leave->end_date ? $leave->end_date->translatedFormat('d F Y') : '',
            'mengalihkan_tugas_kepada_rekan' => $leave->delegation_to ?: '-',
        ...
        @else
            <div class="header-content">
                ...
                <div class="header-text">
                    ...
                    ">
                        {{ replaceVars($line['text'] ?? '', $variableValues) }}
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    @foreach(($template->content_blocks ?? []) as $block)
            ...
            <div class="letter-opening" style="{{ $marginStyle }} {{ $fontStyle }}">
                ...
                <div class="letter-date" style="text-align: {{ $dateConfig['position'] ?? 'right' }}; margin-bottom: {{ $dateConfig['spacing_bottom'] ?? 10 }}mm;">
                    @if($dateConfig['show_place'] ?? false)
                        {{ $dateConfig['place_source'] === 'variable' ? replaceVars($dateConfig['place_text'] ?? '', $variableValues) : ($dateConfig['place_text'] ?? '') }}, 
                    @endif
                    {{ $dateConfig['date_source'] === 'variable' ? formatDateIndo(replaceVars($dateConfig['date_variable'] ?? '', $variableValues)) : formatDateIndo($dateConfig['date_manual'] ?? '') }}
                </div>
                @endif
                ...
                @php
                    ...
                    if (($slot['source'] ?? 'manual') === 'variable') {
                        ...
                        $slotText = replaceVars('{{' . $varName . '}}', $variableValues);
                    }
                @endphp
        ...
        @elseif($block['type'] === 'text' || $block['type'] === 'paragraph')
            <div class="content-block" style="{{ $marginStyle }} {{ $fontStyle }}">
                {!! nl2br(e(replaceVars($block['content'] ?? '', $variableValues))) !!}
            </div>
        @elseif($block['type'] === 'field-group')
            ...
            <div class="field-group" style="{{ $marginStyle }} {{ $fontStyle }}">
                ...
                <div class="field-group-row">
                    ...
                    <span class="field-value">{{ replaceVars($item['value'] ?? '', $variableValues) }}</span>
                </div>
                @endforeach
            </div>
    ...
    <div class="signature-container">
        @php
            $columnCount = (int) explode('-', $signatureSettings['layout'] ?? '3-column')[0];
            $columns = [];
            ...
            // Signature data - Pemohon, Menyetujui (HR), Delegasi
            $employeeName = $leave->employee->user->name ?? '[Nama Pemohon]';
            $employeeNip = $leave->employee->user->nip ?? '[NIP]';
            
            // HR Approver
            $approverName = $leave->approver->name ?? '[Nama HR]';
            $approverNip = $leave->approver->nip ?? '[NIP]';
            
            // Delegasi - dari delegation_employee
            $delegationName = $leave->delegationEmployee->user->name ?? ($leave->delegation_to ?? '[Nama Delegasi]');
            $delegationNip = $leave->delegationEmployee->user->nip ?? '[NIP]';
        @endphp
        ...
        <div class="signature-grid">
            ...
            <div class="signature-column" style="width: {{ 100 / $columnCount }}%;">
                @foreach($slots as $slotIndex => $slot)
                    @php
                        // Kolom 0 = Pemohon, Kolom 1 = Menyetujui (HR), Kolom 2 = Delegasi
                        if ($colIndex === 0) {
                            $name = $employeeName;
                            $nip = $employeeNip;
                        } elseif ($colIndex === 1) {
                    ...
                    <div class="signature-slot" style="text-align: {{ $slot['text_align'] ?? 'center' }}; font-size: {{ $slot['font_size'] ?? 12 }}pt;">
                        ...
                        <div>{{ replaceVars($slot['label_above'], $variableValues) }}</div>
                        @endif
                        @if($slot['label_position'] ?? null)
                        <div>{{ replaceVars($slot['label_position'], $variableValues) }}</div>
                        @endif
