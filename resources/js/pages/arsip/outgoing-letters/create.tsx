...
import { 
    ...
    ZoomIn, ZoomOut, RotateCcw, PenTool, Zap, GripVertical, AlertCircle
} from 'lucide-react';
...
const breadcrumbs = [
    ...
    { title: 'Buat Surat', href: '#' },
];

export default function Create({ templates, users }: Props) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        signatories: [] as { user_id: number; slot_id: string; sign_order: number }[],
    });
    ...
    useEffect(() => {
        if (data.template_id) {
            const template = templates.find(t => t.id?.toString() === data.template_id);
            setSelectedTemplate(template || null);
            
            // Initialize signatories based on template slots
            if (template?.signature_settings?.slots?.length) {
                setData('signatories', template.signature_settings.slots.map((slot, index) => ({
                    user_id: 0,
                    slot_id: slot.id,
                    sign_order: index,
                })));
            ...
            // Stay on template tab to fill subject and date first
            // Don't auto-switch to variables tab
        } else {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!data.template_id) {
            newErrors.push('Template surat harus dipilih');
        }
        if (!data.subject.trim()) {
            newErrors.push('Perihal surat harus diisi');
        }
        if (!data.letter_date) {
        ...
        const requiredVars = selectedTemplate?.variables?.filter(v => v.required && (!v.source || v.source === 'manual')) || [];
        for (const v of requiredVars) {
            if (!data.variable_values[v.key] || data.variable_values[v.key].toString().trim() === '') {
                newErrors.push(`Variabel "${v.label || v.key}" harus diisi`);
            }
        }
        
        // Check signatories
        const slotsWithNoUser = data.signatories.filter(s => !s.user_id || s.user_id === 0);
        if (slotsWithNoUser.length > 0) {
            newErrors.push(`${slotsWithNoUser.length} slot tanda tangan belum dipilih penandatangannya`);
        }
    ...
    const updateSignatory = (slotId: string, userId: number) => {
        ...
        if (existingIndex >= 0) {
            ...
            const slotIndex = selectedTemplate?.signature_settings?.slots?.findIndex(s => s.id === slotId) ?? 0;
    ...
    const manualVariables = selectedTemplate?.variables?.filter(v => !v.source || v.source === 'manual') || [];
            ...
            <form onSubmit={handleSubmit} className="h-[calc(100vh-64px)] flex flex-col">
                ...
                <div className="h-14 border-b bg-background flex items-center justify-between px-4 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-sm font-semibold leading-none">Buat Surat Keluar</h1>
                    ...
                    <div className="flex items-center gap-2">
                        <Button type="submit" size="sm" disabled={processing || !selectedTemplate}>
                            {processing ? (
                        ...
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full">
                            <TabsList className="w-full h-auto p-0 bg-transparent justify-start gap-0 rounded-none border-b">
                                ...
                                <TabsTrigger 
                                    ...
                                    disabled={!selectedTemplate}
                                >
                                    TTD {selectedTemplate?.signature_settings?.slots?.length ? `(${selectedTemplate.signature_settings.slots.length})` : ''}
                                </TabsTrigger>
                            </TabsList>
                                    ...
                                    <TabsContent value="template" className="m-0 space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-xs">
                                                Pilih Template <span className="text-destructive">*</span>
                                            </Label>
                                            <Select
                                                value={data.template_id}
                                                onValueChange={(value) => setData('template_id', value)}
                                            >
                                                <SelectTrigger className={`h-9 ${errors.template_id ? 'border-destructive' : ''}`}>
                                            ...
                                            <>
                                                ...
                                                <div className="bg-muted/40 rounded-lg p-3 space-y-2 text-sm">
                                                    <div className="flex justify-between">
                                                        ...
                                                        <span className="font-mono text-xs">{selectedTemplate.code}</span>
                                                    </div>
                                                    {selectedTemplate.category && (
                                                        <div className="flex justify-between">
                                                            <span className="text-muted-foreground text-xs">Kategori:</span>
                                                            <span className="text-xs">{selectedTemplate.category}</span>
                                                        </div>
                                                    )}
                                    ...
                                    <TabsContent value="variables" className="m-0 space-y-4">
                                        {selectedTemplate && (
                                            <>
                                                ...
                                                {/* Auto variables info */}
                                                {(selectedTemplate.variables || []).filter(v => v.source && v.source !== 'manual').length > 0 && (
                                                    <div className="mt-4 pt-4 border-t">
                                                        ...
                                                        <div className="space-y-1">
                                                            {(selectedTemplate.variables || []).filter(v => v.source && v.source !== 'manual').map((variable) => (
                                    ...
                                    <TabsContent value="signatories" className="m-0 space-y-4">
                                        {selectedTemplate && (
                                            <>
                                                ...
                                                <div className="space-y-3">
                                                    {(selectedTemplate.signature_settings?.slots || []).map((slot, index) => {
                                                    ...
                                                    {(!selectedTemplate.signature_settings?.slots?.length) && (
                                                        <div className="text-center py-6 text-xs text-muted-foreground border border-dashed rounded-lg">
                        ...
                        <div className="flex-1 overflow-auto">
                            {selectedTemplate ? (
                                <div className="min-h-full flex items-start justify-center py-6 px-4">
                                    <TemplatePreview
                                        pageSettings={selectedTemplate.page_settings}
                                        headerSettings={selectedTemplate.header_settings}
                                        contentBlocks={selectedTemplate.content_blocks}
                                        signatureSettings={selectedTemplate.signature_settings}
                                        footerSettings={selectedTemplate.footer_settings}
