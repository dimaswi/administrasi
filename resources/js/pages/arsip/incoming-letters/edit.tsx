...
export default function Edit({ letter, organizationUnits, categories }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (data.description) formPayload.description = data.description;
        ...
        router.post(route('arsip.incoming-letters.update', letter.id), formPayload, {
            forceFormData: true,
            onError: (errors) => {
    ...
    const hasExistingFile = letter.file_path && !data.file;
                ...
                <div className="h-14 border-b bg-background flex items-center justify-between px-4 shrink-0">
                    <div className="flex items-center gap-3">
                        <Button 
                            ...
                            variant="ghost" 
                            size="sm"
                            onClick={() => router.visit(route('arsip.incoming-letters.show', letter.id))}
                        >
                            <ArrowLeft className="h-4 w-4" />
                        </Button>
                        <Separator orientation="vertical" className="h-6" />
                        <div>
                            <h1 className="text-sm font-semibold leading-none">Edit Surat Masuk</h1>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="border-2 border-dashed rounded-lg p-4 text-center hover:border-primary/50 transition-colors">
                                        ...
                                        >
                                            ...
                                            {data.file ? 'Ganti File Baru' : hasExistingFile ? 'Ganti File' : 'Pilih File'}
                                        </Button>
                                        {data.file && (
                    ...
                    <div ref={previewContainerRef} className="flex-1 bg-muted/30 flex flex-col overflow-hidden">
                        ...
                        <div className="h-10 border-b bg-background flex items-center justify-between px-4 shrink-0">
                            <span className="text-xs text-muted-foreground">
                                {previewUrl ? 'Preview File Baru' : hasExistingFile ? 'Preview Dokumen Tersimpan' : 'Tidak ada file'}
                            </span>
                            {(previewUrl || hasExistingFile) && (
