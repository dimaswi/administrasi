...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (config('database.default') === 'pgsql') {
            ...
            DB::statement("ALTER TABLE leaves ALTER COLUMN status TYPE VARCHAR(50)");
            
            // Add new check constraint with all status values
            DB::statement("ALTER TABLE leaves ADD CONSTRAINT leaves_status_check CHECK (status::text = ANY (ARRAY['draft'::text, 'pending'::text, 'pending_delegation'::text, 'pending_supervisor'::text, 'pending_hr'::text, 'pending_director_sign'::text, 'approved'::text, 'rejected'::text, 'cancelled'::text]))");
        } else {
            // MySQL: Modify enum
            DB::statement("ALTER TABLE leaves MODIFY COLUMN status ENUM('draft', 'pending', 'pending_delegation', 'pending_supervisor', 'pending_hr', 'pending_director_sign', 'approved', 'rejected', 'cancelled') DEFAULT 'draft'");
        }
    }
